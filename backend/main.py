...
def get_global_markets():
    try:
        ...
        for ticker in ["GC=F", "MGC=F"]:
            try:
                temp_hist = yf.Ticker(ticker).history(period=GOLD_HISTORY_PERIOD)[
                    "Close"
                ]
                if len(temp_hist) >= EMA_SLOW:
                    gold_hist = temp_hist
                    break
            except:
                continue
...
def get_period_trade_bounds(period_key, now):
    ...
    if period_key == "WD_Late_Night":
        return datetime.combine(current_date, dt_time(0, 0, 0)), datetime.combine(
            current_date, dt_time(1, 59, 59)
        )
    if period_key == "WD_Morning":
        return datetime.combine(current_date, dt_time(6, 0, 0)), datetime.combine(
            current_date, dt_time(11, 59, 59)
        )
    if period_key == "WD_Afternoon":
        return datetime.combine(current_date, dt_time(12, 0, 0)), datetime.combine(
            current_date, dt_time(17, 59, 59)
        )
    if period_key == "WD_Evening":
        return datetime.combine(current_date, dt_time(18, 0, 0)), datetime.combine(
            current_date, dt_time(23, 59, 59)
        )
    if period_key == "WE_Active":
        return datetime.combine(current_date, dt_time(9, 30, 0)), datetime.combine(
            current_date, dt_time(17, 29, 59)
        )
    return None, None
...
def push_log_to_server(
    action, price, reason, amount, current_nav, ai_action, user_action
):
    if not ENABLE_UNIVERSITY_API:
        return
    payload = {
        "action": action,
...
def run_ai_analysis_logic():
    ...
    if target_trades == 0:
        ...
        current_trades < target_trades
        and minutes_remaining <= URGENCY_MINUTES_THRESHOLD
    ):
        dynamic_quota = (
    ...
    while True:
        try:
            ...
            if res and "Ask965" in res and "Bid965" in res:
                ...
                history.append(
                    {"timestamp": now_str, "price": sell_price, "buy": buy_price}
                )
...
def get_latest_news_api():
    try:
        feed = feedparser.parse("https://www.bangkokpost.com/rss/data/business.xml")
        news_list = [
            {"title": e.get("title"), "link": e.get("link")} for e in feed.entries[:5]
        ]
